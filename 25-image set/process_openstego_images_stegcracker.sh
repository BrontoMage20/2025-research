...
mkdir -p stegcracker_openstego_results
...
#find /home/brontomage20/openstego_stegos -type f -name '*.bmp' | while read -r img; do
#    	echo "==="
#    	echo "Processing: $img"
#    	echo "==="
#    
...
find /home/brontomage20/openstego_stegos -type f -name '*.bmp' | while read -r img; do
   	...
   	outfile="./stegcracker_openstego_results/${filename}.out"
   	logfile="./stegcracker_openstego_results/${filename}_result.txt"

   	#run stegcracker and save results
   	stegcracker "$img" /home/brontomage20/Wordlists/wordlist_updated_no_num.txt 2>&1 | tee "$logfile"

	#stegcracker makes the file in the same directory as the input with .out appended
	stegcracker_output="${img}.out"
...
ls -1 stegcracker_openstego_results/*.out 2>/dev/null | while read file; do
	echo "$file"
done
...
echo "summary of successful extractions:"
successful=0
for file in ./stegcracker_openstego_results/*.out; do
