...
stego_dir="/home/brontomage20/openstego_stegos_round_2" #directory containing the stego files
output_dir="/home/brontomage20/openstego_extracted_round_2" #directory for extracted files
...
# Gather all BMP stego files.
files=()
while IFS= read -r -d '' file; do
	files+=("$file")
done < <(find "$stego_dir" -maxdepth 1 -type f -iname '*.bmp' -print0)

total=${#files[@]}

if [ "$total" -eq 0 ]; then
	echo "No .bmp stego files found in directory: $stego_dir"
	exit 1
fi
